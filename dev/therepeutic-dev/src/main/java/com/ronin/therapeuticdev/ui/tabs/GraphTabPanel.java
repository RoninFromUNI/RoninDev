...
public class GraphTabPanel extends JBPanel<GraphTabPanel> {
    ...
    private JBPanel<?> createToolbar() {
        ...
        // spacer
        toolbar.add(Box.createHorizontalStrut(JBUI.scale(12)));

        // layout toggle — switches between vertical (2-col grid) and
        // horizontal (single row, scrolls sideways). the label updates
        // to show what mode you'll switch TO, not what youre currently in.
        JButton layoutBtn = new JButton("Horizontal");
        layoutBtn.addActionListener(e -> {
            graphCanvas.toggleLayout();
            layoutBtn.setText(graphCanvas.isHorizontalMode() ? "Vertical" : "Horizontal");
        });
    ...
    private class GraphCanvas extends JBPanel<GraphCanvas> {
        ...
        /**
         * layout mode toggle.
         * false = vertical (2-column grid, scrolls down) — default
         * true  = horizontal (single row, scrolls right)
         */
        ...
        public void zoom(double delta) {
            ...
            revalidate();  // trigger scroll pane to re-read preferred size
            repaint();
        }
        ...
        protected void paintComponent(Graphics g) {
            ...
            int y = padding;
            int col = 0;
            // vertical = 2 columns wrapping down, horizontal = single row scrolling right
            int maxCols = horizontalMode ? Integer.MAX_VALUE : 2;
            int maxX = 0;  // track rightmost edge
            int maxY = 0;  // track bottommost edge
            ...
            for (Map.Entry<String, List<ClassNode>> entry : packages.entrySet()) {
                ...
                // track content edges
                maxX = Math.max(maxX, x + groupWidth + padding);
                maxY = Math.max(maxY, y + groupHeight + padding);
            }

            // update preferred size so the scroll pane knows the actual
            // content bounds — this is what makes scrolling work.
            // without this the canvas stays at its initial size and
            // anything beyond that is clipped with no scrollbar.
            int scaledW = (int)(maxX * zoomLevel);
            int scaledH = (int)(maxY * zoomLevel);
            Dimension newSize = new Dimension(scaledW, scaledH);
